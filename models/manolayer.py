...
class ManoLayer(Module):
    ...
    def axis2Rmat(self, axis):
        # axis: bs x 45
        joint_num = axis.size(-1) // 3
        rotation_mat = rodrigues_batch(axis.view(-1, 3))
        rotation_mat = rotation_mat.view(-1, joint_num, 3, 3)
        return rotation_mat

    def axis2pca(self, axis):
        # axis: bs x 45
        pca = axis - self.hands_mean.unsqueeze(0)
    ...
    def forward(self, root_rotation, pose, shape, trans=None, scale=None):
        ...
        SE3_j = []
        R = root_rotation.reshape([bs, 3, 3])
        ...
        if scale is not None:
            scale = scale.view(bs, 1, 1)  # bs * 1 * 1
            v_output = v_output * scale
            j_output = j_output * scale

        if trans is not None:
            trans = trans.view(bs, 1, 3)  # bs * 1 * 3
